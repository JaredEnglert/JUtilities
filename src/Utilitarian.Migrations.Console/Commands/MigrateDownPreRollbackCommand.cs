...
{
    public class MigrateDownPreRollbackCommand : ConsoleCommand
    {
        ...
        public MigrateDownPreRollbackCommand(IConnectionStringProvider connectionStringProvider, IRepositoryService repositoryService, IEnumerable<Migration> migrations)
        {
            _connectionStringProvider = connectionStringProvider;
            _repositoryService = repositoryService;
            _migrations = migrations;

            IsCommand("migrate-down-pre-rollback", "Runs the MigrateDownPreRollback method on all eligible migrations. It is Step 3, and used at the beginning of a rollback, if step already occurred.");
        ...
        private async Task RunAsync()
        {
            ...
            Log.Information("Starting MigrationService.MigrateDownPreRollback{0}{0}\tDatabase Type: {1}{0}\tDatabase Name: {2}{0}\tMigration Topic: {3}{0}{4}",
            ...
            await migrationService.MigrateDownPreRollback(_migrationTopic, _versions);

            Log.Information("MigrationService.MigrateDownPreRollback Complete");
        }
    }
}
